...
def saveSkinWeights(characterName, geoList = []):
    ...
    for obj in geoList:
        # wieghts file
        wtFile = os.path.join(mainProjectPath, characterName, skinweightsDir, obj + swExt)
...
def loadSkinWeights(characterName, geoList = []):
    ...
    for wtFile in wtFiles:
        exRes = os.path.splitext(wtFile)

        #check extension format
        if not extRes > 1:
            continue
        #check skin wheigh files
